...
def show_ui():
    ...
    if cmds.window("poseTransferWindow", exists=True):
        cmds.deleteUI("poseTransferWindow")
    
    window = cmds.window("poseTransferWindow", title='Pose Transfer Tool', width=350)
    cmds.columnLayout(adjustableColumn=True)
    cmds.text(label='POSE TRANSFER', align='center', height=35,
              annotation='Transfer pose from first selected rig to the rest of the selection.')
    cmds.button(label='Transfer Selected',
                height=40,
                annotation='Select 2 or more characters.',
